...
void render_clouds(cv::Mat& img, float noise_offset_z) {
    for (int i = 0; i <= subdiv_x; ++i) {
        for (int j = 0; j <= subdiv_z; ++j) {
            float x = base_grid[i][j][0];
            float z = base_grid[i][j][1];
            float nx = i * cloud_noise_scale;
            float nz = j * cloud_noise_scale + noise_offset_z;
            float noise_val = fractal_noise(nx, nz);

            if (noise_val < 0.5f)
                continue; // zona transparente

            float y = cloud_height + (1.0f - noise_val) * cloud_noise_amplitude;
            cv::Vec3f point(x, y, z);
            cv::Point proj = project_point(point);
            if (proj.x != -1)
                cv::circle(img, proj, 1, cv::Scalar(160, 160, 255), -1); // nubes azuladas
        }
    }
}
...
int main() {
    ...
    std::string filename = "terrain_with_clouds_" + std::to_string(epoch) + ".mp4";
    ...
    while (frame_count < total_frames) {
        ...
        cv::imshow("Fractal Terrain with Clouds", final);
        writer.write(final);
